...
**Controls today:** W/S move; A/D turn 10°; click empty → Pointer Lock; click door → open only (no lock); Esc unlock. **Mobile (coarse):** left-bottom virtual D-pad (↑↓ walk, ←→ continuous turn); single-finger drag look; no pointer lock.
